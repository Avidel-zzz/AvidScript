...
2. 在关卡中的拾取物 Actor 上绑定生成的 `.avidscript.json` manifest，或使用 Editor 构建报告自动创建并绑定 `UAvidScriptComponent`。
3. 给玩家 Actor 添加 `Player` Actor Tag，并确保拾取物能够产生 Actor BeginOverlap 事件。

样例只调用自动生成的 `AActor` facade 和公共运行时原语，没有为 `ActorHasTag`、旋转、隐藏或碰撞编写专用宿主胶水。
