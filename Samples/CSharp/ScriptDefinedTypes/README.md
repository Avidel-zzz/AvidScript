...
同一生成包的 body-only 热重载也已测试：候选准备失败时保留原订阅和等待；成功发布后旧回调与等待会被取消，脚本可再次订阅。另有[仅把恢复后倍率改为 `20` 的候选源码](../../../Tests/Fixtures/CSharp/P66B_ScriptDefinedTypesBodyCandidate.cs)：Win64 Editor 自动化确认反射结构不变、包 ID 改变，切换后重新订阅会执行新方法体。可用[版本切换复现脚本](../../../Build/Contracts/TestGeneratedCSharpEventVersionReload.ps1)在 Game 和 PIE 类型的测试 World 各重跑一次；Editor 真实 Play 操作、Cook/Shipping 切换和长时间运行仍需另外验收。
