...
- **FVector ref/out：** P65.D11 将 generated S1 从 `1026.61 ns` 降至 **`103.64 ns`**，相对 Puerts static 为 **`0.144x`**；十项 UE micro 的 P50/P95 几何均值改善到 **`0.810x / 0.870x`**，但 object、callback、纯执行与 AngelScript 同语义门禁仍未关闭。见[D11 报告](Docs/Phase65/P65.D11_Generated_Vector_RefOut_Performance.md)。
...
P65.D11 已把通用实例 `FVector ref/out` 接入 generated typed-host 与 prepared native writeback。
真实 packaged host 的 5 进程正式结果为 **`103.64 ns`**，较 D10 同路径提速约 **`9.91x`**，
相对冻结 Puerts static 快约 **`6.92x`**；十项 UE micro 的 generated S1 P50/P95 几何均值为
**`0.810x / 0.870x`**、胜项 `6/10`。P65-D03 仍保持未关闭，下一批处理 UObject roundtrip、
callback 与相同 WASM 执行层。详见
[P65.D11 Generated FVector Ref/Out](Docs/Phase65/P65.D11_Generated_Vector_RefOut_Performance.md)。
