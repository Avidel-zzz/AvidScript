...
| 异步 | 计时器、下一帧、资源加载；受支持的静态异步入口可跨 `await` 保留局部对象及同步函数返回的闭包 | 在 async 方法内创建捕获闭包、实例异步方法[仍在实现，附对象保存示例](Docs/Phase66/P66.B_Persistent_Managed_Roots.md#c-异步状态对象)；不能使用任意 awaiter |
