...
- **C# 子集**：支持成员内静态、同步、非泛型局部函数；无完整 .NET Runtime、任意 awaiter 或异常系统；暂不支持捕获局部函数、`event +=`、lambda/closure，使用显式 bind/subscribe 与 `ExecuteX/BroadcastX`。
