...
- **异常：** 专用测试入口可执行 `try { throw new System.Exception(); } catch (System.Exception) { throw; } finally { Count = Count + 1; }` 等受限写法；普通脚本构建尚未启用。见[当前实现范围](Docs/Phase66/P66.C3_Language_Error_Channel_Contract.md)。
