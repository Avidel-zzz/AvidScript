...
- `Task<int>` 支持直接 `await` 同源静态方法、按声明顺序传入值参数，以及取消终态的链式传播。Task 变量和其他 `Task<T>` 尚未支持；异步错误还不能按普通 C# 的 `catch` / `finally` 语义处理。详见 [Task 结果合同](Docs/Phase66/P66.C4_Task_Result_Contract.md)。
