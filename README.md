...
| 异步 | 计时器、下一帧、资源加载；受支持的静态异步入口可跨 `await` 保留局部对象，以及方法内创建的闭包（会记住并共享外层变量的函数） | 实例异步方法仍未接通；不能使用任意 awaiter。[共享变量的代码示例](Docs/Phase66/P66.B_Async_Closure_Scopes.md#环境分配与状态恢复) |
