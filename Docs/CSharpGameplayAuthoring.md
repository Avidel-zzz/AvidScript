...
> 当前状态: 以 PC / Windows Editor 为主, 可以用受限 C# 子集生成 direct ABI WASM, 绑定到选中的 UE Actor, 并接入 `BeginPlay` / `Tick` 生命周期。`.avid` 不是当前必经入口。
...
2. 在这两个函数里调用 `Actor.SetLocation(float x, float y, float z)`。
3. 从 Editor 生成默认 C# profile。
4. 选中关卡里的 Actor。
5. 从 Editor 构建 C# -> WASM/report/manifest, 并绑定到该 Actor 的 `UAvidScriptComponent`。
6. PIE/运行时由组件在 UE `BeginPlay` 时加载 WASM 并调用 `avid_on_begin_play`, 在每帧 Tick 时调用 `avid_on_tick(deltaSeconds)`。

这意味着你可以先写“Actor 位置变化”级别的游戏逻辑, 用它验证 C# 脚本语言到 UE 事件流的完整闭环。
...
`BeginPlay()` 对应 UE 组件开始运行的时机; `Tick(float deltaSeconds)` 对应 UE 组件 Tick, 其中 `deltaSeconds` 由宿主传入。

## 支持的 C# 子集

Phase22 不扩展语法, 当前仍沿用 C# source adapter 的 `actor_lifecycle_v1` 子集:
...
- 除 `Actor.SetLocation` 之外的 host binding。
- 移动端构建验证。

当前的目标是先把 UE 事件流、WASM ABI、组件绑定、Editor 工作流跑通, 再逐步扩大 C# 可写能力。
...
4. PIE 或运行关卡。
5. 观察 Actor 是否在 BeginPlay 后移动到初始位置, 并在 Tick 后根据 `deltaSeconds` 更新位置。
6. 如需确认生命周期调用, 查看 UE log 中的 `AvidScript component start` 与首次 `AvidScript component tick`。

## 常见诊断

Phase22 后, C# profile/template/build/bind 输出会统一成 title/body/details 形式, 并带 `Next action`。常见类别如下:
...
当前 PC Editor 路线已经能验证 C# -> WASM -> UE Actor 生命周期闭环, 但还不是完整产品化脚本语言体验。下一步应优先扩展 C# 可写能力与绑定面, 例如状态字段、更多 Actor API、输入/碰撞/Timer 事件、错误定位到源文件行列, 再进入打包和移动端验证。
