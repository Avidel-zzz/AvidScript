...
runtime 会在返回 slot/generation 前验证 owner handle。缺失、无效或 stale generation 都会 fail closed。自动化测试已让目标 Actor 位于非 1 号槽位，证明 typed API 不依赖固定句柄。
