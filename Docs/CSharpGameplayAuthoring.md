...
> 当前状态: 以 PC / Windows Editor 为主, 可以用受限 C# 子集生成 direct ABI WASM, 绑定到选中的 UE Actor, 并接入 `BeginPlay` / `Tick` 生命周期。Phase24 后, C# 已支持 `Actor.SetLocation(...)`、`Actor.AddLocationOffset(...)`、`private static float` 状态字段、字段赋值和字段累加; `.avid` 不是当前必经入口。
...
6. 从 Editor 生成默认 C# profile, 构建 C# -> WASM/report/manifest, 并绑定到选中 Actor。
7. PIE/运行时由 `UAvidScriptComponent` 在 UE `BeginPlay` 时加载 WASM 并调用 `avid_on_begin_play`, 每帧 Tick 时调用 `avid_on_tick(deltaSeconds)`。

这意味着你已经可以先写“Actor 初始化 + Tick 状态累积 + Actor 状态更新”级别的游戏逻辑, 用 C# 验证脚本语言到 UE 事件流的闭环。
...
`BeginPlay()` 对应 UE 组件开始运行的时机; `Tick(float deltaSeconds)` 对应 UE 组件 Tick, 其中 `deltaSeconds` 由宿主传入。上面的示例会在 BeginPlay 时把 Actor 移动到 `(100, 200, 300)`, 然后每帧根据脚本内累计的 `ElapsedSeconds` 计算绝对位置。自动化测试会连续 Tick 两次, 验证第一次 Tick 后 X 为 `102`, 第二次 Tick 后 X 为 `104`, 证明字段状态会跨 Tick 保留。

## 支持的 C# 子集

Phase24 后, C# source adapter 的当前子集为 `actor_lifecycle_v3`:
...
| `source_adapter_failed` | C# 源码超出当前受限子集 | 先改回 `actor_lifecycle_v3` 支持的字段、赋值和 Actor 调用形式。 |
...
当前 PC Editor 路线已经能验证 C# -> WASM -> UE Actor 生命周期闭环, 并支持初始化位置、Tick 增量位移、静态 float 状态和状态化位置计算, 但还不是完整产品化脚本语言体验。下一步应优先扩展 C# 可写能力与绑定面, 例如更多 Actor API、输入、碰撞/Overlap/Hit、Timer、错误定位到源文件行列, 再进入打包和移动端验证。
