...
1. 先冻结架构契约，再连续实现。
2. 以完整功能组划分批次，不以单个测试或文件划分任务。
3. 阶段中途只处理阻塞性高风险问题。
4. 普通缺陷进入阶段债务清单，在代码冻结后集中修复。
5. 先审查、后执行昂贵验收，避免审查修改使验证失效。
6. 只有最终 Gate 可以将 Phase 标记为完成。
...
每个 Phase 默认拆分为 3 至 5 个实现批次，每批次约 60 至 120 分钟，并以可交付功能为边界。批次应同时包含完成该功能所需的产品代码、必要测试代码和最小文档记录。
...
代码冻结后的普通修复不应逐项触发完整构建。只有修复本身暴露新的 Blocker 或 Critical 风险时才执行额外探针。
...
不再为单个测试、一次构建修复或微小文档调整建立碎片提交。实现提交仍需保持可审查，不允许把多个无关模块压成一个无法理解的提交。
