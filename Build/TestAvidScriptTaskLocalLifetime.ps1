...
$runRoot = Join-Path $projectRoot "Saved/AvidScriptTaskLocalLifetime/$runId"
$null = New-Item -ItemType Directory -Path $runRoot -Force
$oldCliHome = $env:DOTNET_CLI_HOME
$oldFixtures = $env:AVIDSCRIPT_TASK_LIFETIME_FIXTURE_DIR
Push-Location $pluginRoot
try {
    $env:DOTNET_CLI_HOME = Join-Path ([IO.Path]::GetTempPath()) 'AvidScriptTaskLifetimeCliHome'
    $env:AVIDSCRIPT_TASK_LIFETIME_FIXTURE_DIR = Join-Path $runRoot 'Fixtures'
    $sdk = & $DotNetPath --version
    if ($LASTEXITCODE -ne 0 -or $sdk -cne '8.0.416') { throw "Expected SDK 8.0.416, got $sdk" }
    $managed = @(& $DotNetPath run --project Tools/AvidScript.CSharpGuest.Tests/AvidScript.CSharpGuest.Tests.csproj -c Release -- --task-local-lifetime)
    $managedExit = $LASTEXITCODE
    $managed | Set-Content -LiteralPath (Join-Path $runRoot 'managed.log') -Encoding utf8
    $match = [regex]::Match(($managed -join "`n"), 'AvidScript.CSharpGuest.Tests.TaskLocalLifetime: (\d+)/(\d+) passed')
    if ($managedExit -ne 0 -or -not $match.Success -or $match.Groups[1].Value -cne $match.Groups[2].Value) {
        throw "Task lifetime fixture generation failed: $runRoot"
    }
    if (-not $SkipBuild) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Task lifetime Automation failed: $logPath" }
    ...
    $cases = [regex]::Matches($log, 'task-lifetime backend=\d+ scenario=[a-z-]+ mode=\d+ result=-?\d+ resumes=\d+').Count
    if ($passed -ne $tests.Count -or $cases -ne 134 -or
        ...
        throw "Task lifetime evidence incomplete: tests=$passed cases=$cases log=$logPath"
    }
    [ordered]@{
        schema_version = 1
        semantic_version = '45/1.54'
        guest_ir_version = '25/1.24'
        managed_passed = [int]$match.Groups[1].Value
        native_tests_passed = $passed
        vm_scenarios_passed = $cases
        fixture_directory = $env:AVIDSCRIPT_TASK_LIFETIME_FIXTURE_DIR
        automation_log = $logPath
    } | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $runRoot 'results.json') -Encoding utf8
    Write-Output "Task local lifetime: managed=$($match.Groups[1].Value)/$($match.Groups[2].Value), native=$passed/3, Wasmtime/WAMR=$cases/134; evidence=$runRoot"
}
finally {
    Pop-Location
    $env:DOTNET_CLI_HOME = $oldCliHome
    $env:AVIDSCRIPT_TASK_LIFETIME_FIXTURE_DIR = $oldFixtures
}
