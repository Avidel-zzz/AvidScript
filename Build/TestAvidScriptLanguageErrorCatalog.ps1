...
try {
    ...
    & node 'Tools/AvidScript.CSharpGuest.Tests/RunThrowProducerWasm.cjs' (Join-Path $FixtureDirectory 'typed-catch.wasm')
    if ($LASTEXITCODE -ne 0) { throw 'C# typed catch Node WASM probe failed.' }
    ...
    & node 'Tools/AvidScript.CSharpGuest.Tests/RunThrowProducerWasm.cjs' (Join-Path $FixtureDirectory 'side-effect-throw-finally.wasm')
    if ($LASTEXITCODE -ne 0) { throw 'C# bounded side-effecting throw decision Node WASM probe failed.' }
    ...
    & node 'Tools/AvidScript.CSharpGuest.Tests/RunThrowProducerWasm.cjs' (Join-Path $FixtureDirectory 'consecutive-nested-cleanup-replaces-error.wasm')
    if ($LASTEXITCODE -ne 0) { throw 'C# consecutive-nested-cleanup-replaces-error Node WASM probe failed.' }
    ...
    & node 'Tools/AvidScript.CSharpGuest.Tests/RunThrowProducerWasm.cjs' (Join-Path $FixtureDirectory 'conditional-void-guard.wasm')
    if ($LASTEXITCODE -ne 0) { throw 'C# conditional void guard Node WASM probe failed.' }
...
$Found = [regex]::Matches($Log, "Found 5 automation tests based on '$([regex]::Escape($TestPrefix))'").Count
...
$GeneratedSuccess = [regex]::Matches($Log,
    'Test Completed\. Result=\{Success\} Name=\{GeneratedUFunction\} Path=\{AvidScript\.Runtime\.LanguageErrorCatalog\.GeneratedUFunction\}').Count
$TaskFaultSuccess = [regex]::Matches($Log,
    'Test Completed\. Result=\{Success\} Name=\{TaskFaultVmImport\} Path=\{AvidScript\.Runtime\.LanguageErrorCatalog\.TaskFaultVmImport\}').Count
...
if ($Found -ne 1 -or $LoadSuccess -ne 1 -or $RealSuccess -ne 1 -or $HandledSuccess -ne 1 -or $GeneratedSuccess -ne 1 -or $TaskFaultSuccess -ne 1 -or $Failed -ne 0 -or $Complete -ne 1 -or $Exit -lt 1) {
    throw "Language-error catalog Automation evidence incomplete: found=$Found load=$LoadSuccess real=$RealSuccess handled=$HandledSuccess generated=$GeneratedSuccess task_fault=$TaskFaultSuccess failed=$Failed complete=$Complete exit=$Exit log=$LogPath"
}
Write-Output "AvidScript.Runtime.LanguageErrorCatalog: 5/5 passed; log=$LogPath"
