...
$GuestDirectory = Join-Path $ProjectRoot 'Saved/AvidScriptFinallyTests/GuestFixtures'
...
try {
    ...
    if ($LASTEXITCODE -ne 0 -or $Sdk -ne '8.0.416') {
        throw "Finally cleanup fixtures require SDK 8.0.416, got $Sdk"
    }
    & pwsh -NoProfile -File (Join-Path $PluginRoot 'Tools/AvidScript.CSharpGuest.Tests/RunFinallyComparison.ps1') `
        -DotNetPath $DotNet -OutputDirectory $GuestDirectory
    if ($LASTEXITCODE -ne 0) { throw 'Finally cleanup .NET/WASM comparison failed.' }
}
finally {
...
$LogPath = Join-Path $ProjectRoot "Saved/Logs/AvidScript_FinallyCleanup_$RunId.log"
$TestName = 'AvidScript.Runtime.FinallyCleanup'
...
if ($LASTEXITCODE -ne 0) { throw "Finally cleanup Automation exited with $LASTEXITCODE. Log: $LogPath" }
$Log = Get-Content -Raw -LiteralPath $LogPath
$Found = [regex]::Matches($Log, "Found 1 automation tests based on '$([regex]::Escape($TestName))'").Count
$Success = [regex]::Matches($Log, 'Test Completed\. Result=\{Success\} Name=\{FinallyCleanup\} Path=\{AvidScript\.Runtime\.FinallyCleanup\}').Count
...
if ($Found -ne 1 -or $Success -ne 1 -or $Failed -ne 0 -or $Complete -ne 1 -or $Exit -lt 1) {
    throw "Finally cleanup Automation evidence incomplete: found=$Found passed=$Success failed=$Failed complete=$Complete exit=$Exit log=$LogPath"
}
Write-Output "AvidScript.Runtime.FinallyCleanup: 1/1 passed; log=$LogPath"
