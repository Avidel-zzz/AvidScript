...
if ($RuntimeAutomation) {
    ...
    $Success = [regex]::Matches($Log, 'Test Completed\. Result=\{Success\} Name=\{(Ownership|HostAbi|StaticStorage|GeneratedGuest|CSharpClosures|GenericMembers|BorrowedReferences)\} Path=\{AvidScript\.Runtime\.ManagedHeap\.\1\}').Count
    ...
    if ($Found -ne 1 -or $Success -ne 7 -or $Failed -ne 0 -or $Complete -ne 1 -or $Exit -lt 1) {
    ...
    Write-Output "AvidScript.ManagedHeap.RuntimeAutomation: 7/7 passed; log=$LogPath"
}
