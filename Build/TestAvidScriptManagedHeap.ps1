...
if ($RuntimeAutomation) {
    ...
    $Success = [regex]::Matches($Log, 'Test Completed\. Result=\{Success\} Name=\{(Ownership|HostAbi|GeneratedGuest)\} Path=\{AvidScript\.Runtime\.ManagedHeap\.\1\}').Count
    ...
    if ($Found -ne 1 -or $Success -ne 3 -or $Failed -ne 0 -or $Complete -ne 1 -or $Exit -lt 1) {
    ...
    Write-Output "AvidScript.ManagedHeap.RuntimeAutomation: 3/3 passed; log=$LogPath"
}
