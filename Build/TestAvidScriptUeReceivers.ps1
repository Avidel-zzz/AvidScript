...
try {
    ...
    if ($CompilerExit -ne 0 -or ($CompilerOutput -join "`n") -notmatch 'AvidScript.CSharpGuest.Tests.UeReceivers: 10/10 passed') {
...
if ($Found -ne 1 -or $Passed.Count -ne 9 -or @($Passed | ForEach-Object { $_.Groups[1].Value } | Select-Object -Unique).Count -ne 9 -or $Failed -ne 0 -or $Complete -ne 1 -or $Exit -lt 1) {
...
Write-Output "AvidScript.UeReceivers.RuntimeAutomation: 9/9 passed; log=$LogPath"
