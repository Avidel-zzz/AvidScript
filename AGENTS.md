...
- Current C# source adapter subset `actor_lifecycle_v5`: `BeginPlay()`, `Tick(float deltaSeconds)`, optional `EndPlay()`, `private static float` fields, field assignment/accumulation, typed `FVector` calls through `UE.Self`/`AActor`, legacy `Actor.SetLocation(...)` / `Actor.AddLocationOffset(...)`, numeric literals, `deltaSeconds`, field references, multiplication, and addition.
