...
- Current C# source adapter subset `actor_lifecycle_v7`: `BeginPlay()`, `Tick(float deltaSeconds)`, optional `EndPlay()`, `private static float` state, handle-backed `UE.Self`/`AActor`, typed location and rotation reads/writes, shared `FVector`/`FRotator` three-component locals and addition, legacy `Actor.*` facade, numeric literals, `deltaSeconds`, field references, multiplication, and addition.
