...
- Current C# source adapter subset `actor_lifecycle_v8`: `BeginPlay()`, `Tick(float deltaSeconds)`, optional `EndPlay()`, `private static float` state, handle-backed `UE.Self`/`AActor`, typed location/rotation/scale reads and writes, shared `FVector`/`FRotator` three-component locals and addition, read-only `FTransform` snapshot projection, legacy `Actor.*` facade, numeric literals, `deltaSeconds`, field references, multiplication, and addition.
