...
- Current C# source adapter subset `actor_lifecycle_v4`: `BeginPlay()`, `Tick(float deltaSeconds)`, optional `EndPlay()`, `private static float` fields, field assignment/accumulation, `Actor.SetLocation(...)`, `Actor.AddLocationOffset(...)`, numeric literals, `deltaSeconds`, field references, multiplication, and addition.
