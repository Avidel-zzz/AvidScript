...
- Current C# source adapter subset `actor_lifecycle_v6`: `BeginPlay()`, `Tick(float deltaSeconds)`, optional `EndPlay()`, `private static float` fields, field assignment/accumulation, handle-backed `UE.Self`/`AActor`, `GetActorLocation()` typed read, local `FVector`, component-wise vector addition, typed location writes, legacy `Actor.*` facade, numeric literals, `deltaSeconds`, field references, multiplication, and addition.
