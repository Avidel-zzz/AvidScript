...
- Semantic schema v2 / semantic version 1.1 is the current typed-operation contract. Binary, unary, compound, and increment/decrement operations retain stable operator_kind plus checked/lifted/postfix flags.
...
- Statement conditional, loop, branch, and coalesce operations remain explicit unsupported operations in P40.2; P40.3 must replace that gate only when stable CFG/control semantics are projected. Ternary conditional expressions remain supported.
