...
internal static class CSharpSemanticInputValidator
{
    ...
    private static bool ValidateTaskLocalOwnership(SemanticDocument document)
    {
        if (document.SchemaVersion is not (SemanticContract.TaskLocalSchemaVersion
            ...
            or SemanticContract.AsyncLanguageErrorSchemaVersion or SemanticContract.AsyncExceptionFlowSchemaVersion or SemanticContract.DirectAwaitCleanupSchemaVersion or SemanticContract.AsyncCancellationFlowSchemaVersion))
        ...
        foreach (SemanticAsyncMethod method in document.AsyncMethods)
        {
            ...
            string[] owned = method.TaskLocalSymbolIds?.OrderBy(id => id, StringComparer.Ordinal).ToArray()
                ?? locals;
            ...
            if (owned.Length > SemanticAsyncInvocationValidator.MaximumTaskLocalsPerMethod
                ...
                || owned.Any(id => method.Segments.SelectMany(segment => segment.Statements)
        ...
        foreach (SemanticAsyncSegment segment in method.Segments)
        {
            ...
            foreach (SemanticAsyncStatement statement in segment.Statements)
            {
                ...
                if (statement.TargetSymbolId is { } targetSymbolId
                    && !(document.SchemaVersion
                        ...
                        && priorType == statement.Operation.TypeId)
                    && (!TryAddMethodLocal(
                            symbolsById,
                        method.MethodSymbolId,
                        targetSymbolId,
            ...
            bool validTransfer = transfer.Kind switch
            {
                ...
                SemanticAsyncMethod.AwaitTransferKind =>
                    ...
                    && transfer.Condition is null
                    && validTargets.Contains(transfer.PrimaryTarget)
                    && (exceptionFlow && transfer.SecondaryTarget >= 0
                        ? validTargets.Contains(transfer.SecondaryTarget)
                        ...
                        : exceptionFlow && document.SchemaVersion is
                            (SemanticContract.DirectAwaitCleanupSchemaVersion or SemanticContract.AsyncCancellationFlowSchemaVersion)
                            && segment.AwaitSite.ProducerKind is "delay" or "next_tick"
                            && transfer.CancellationTarget is not null
                            ? transfer.SecondaryTarget == -1
                                && transfer.CancellationTarget is int directCancellationTarget
                                && validTargets.Contains(directCancellationTarget)
                        : transfer.SecondaryTarget == -1
                ...
                SemanticAsyncMethod.CatchMatchTransferKind =>
                    ...
                    && validTargets.Contains(transfer.PrimaryTarget)
                    && validTargets.Contains(transfer.SecondaryTarget),
                SemanticAsyncMethod.PropagateFaultTransferKind or
                ...
                SemanticAsyncMethod.PropagateExceptionTransferKind =>
                    CSharpTaskResultAbi.SupportsCancellation(document) && exceptionFlow
                    && segment.AwaitSite is null && transfer.Condition is null
    ...
    private static bool IsSupportedContract(int schemaVersion, string semanticVersion)
    {
        return (schemaVersion, semanticVersion) switch
        {
            ...
            (SemanticContract.TaskAliasSchemaVersion, SemanticContract.TaskAliasSemanticVersion) => true,
            (SemanticContract.AsyncLanguageErrorSchemaVersion, SemanticContract.AsyncLanguageErrorSemanticVersion) => true,
            (SemanticContract.AsyncExceptionFlowSchemaVersion, SemanticContract.AsyncExceptionFlowSemanticVersion) => true,
