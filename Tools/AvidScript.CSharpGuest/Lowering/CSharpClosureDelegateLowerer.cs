...
internal static class CSharpClosureDelegateLowerer
{
    public static GuestRegister? Create(CSharpFunctionLoweringContext context, SemanticOperation operation, int block, List<GuestInstruction> instructions)
    {
        ...
        {
            GuestRegister? receiver = CSharpBoundDelegateLowerer.CreateContext(context, callable, target, block, instructions);
        ...
        else
        {
            ...
            { context.Add("ASCG1024", "Static Guest delegate target cannot carry a receiver."); return null; }
            erased = context.CreateTemporary(CSharpClosureLayout.ObjectType, block)!;
    ...
    private static bool Matches(SemanticDocument document, SemanticCallable callable, SemanticDelegateType signature)
        => signature.ReturnRefKind == "none"
            && (callable.IsStatic || CSharpBoundDelegateLowerer.Methods(document).Contains(callable.MethodSymbolId))
            && !callable.IsConstructor && callable.HasBody && callable.Import is null
