...
function Assert-SemanticContract {
    ...
    Assert-Condition (
        [int]$Build.Report.semantic.schema_version -eq 20 -and
        [string]$Build.Report.semantic.version -ceq "1.22" -and
        [int]$Semantic.schema_version -eq 20 -and
        [string]$Semantic.semantic_version -ceq "1.22") `
        "$Label semantic contract is not 20/1.22"
}
