...
public static class GuestConstantCodec
{
    ...
    public static bool TryEncode(GuestConstant constant, GuestType targetType, out byte[] bytes)
    {
        ...
        switch (constant.Kind)
        {
            ...
            case "float64" when targetType.Size == 8 && targetType.Storage == "f64"
                ...
                return true;
            case "null" when (IsIntegerTarget(targetType, 4) || targetType.Kind is "function_ref" or "managed_ref") && constant.Value is null:
                return true;
            default:
    ...
    {
        return type.Kind is "class_ref" or "factory_ref" or "object_type_ref" or "managed_ref";
    }
}
