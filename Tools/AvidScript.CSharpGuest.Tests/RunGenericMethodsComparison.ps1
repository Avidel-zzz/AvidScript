...
if ($ReferenceRows.Count -ne 7 -or $WasmRows.Count -ne 7 -or @(Compare-Object $ReferenceRows $WasmRows -SyncWindow 0).Count -ne 0) {
    throw ("The .NET and WASM results differ. .NET: {0}; WASM: {1}" -f
        ($ReferenceRows -join ", "), ($WasmRows -join ", "))
}
Write-Output "GenericMethodsDotNetWasm: 7/7 matched"
