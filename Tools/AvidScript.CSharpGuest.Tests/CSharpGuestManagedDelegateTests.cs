...
internal static class CSharpGuestManagedDelegateTests
{
    public static int Run()
    {
        ...
        Require(!CSharpGuestLowerer.Lower(CSharpGuestSemanticFixture.WithoutDispatch(document) with { SchemaVersion = 21, SemanticVersion = "1.23", ClassTypes = Array.Empty<SemanticClassType>() }, new string('f', 64)).Succeeded,
            "old semantic versions must not smuggle new delegate operations");
        CSharpGuestLoweringResult previous = CSharpGuestLowerer.Lower(CSharpGuestSemanticFixture.WithoutDispatch(document) with { SchemaVersion = 21, SemanticVersion = "1.24", ClassTypes = Array.Empty<SemanticClassType>() }, new string('f', 64));
