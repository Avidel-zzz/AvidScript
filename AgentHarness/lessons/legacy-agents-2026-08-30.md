...
- 2026-08-29 P57.12C9 新增 `continuation_result_read` 时只完成了共享静态目录、Wasmtime 通用 adapter 和 Runtime dispatcher，遗漏 WAMR `GetWamrStaticHostFunction` 原生符号，完整 Automation 在首个 WAMR Runtime 测试初始化时以 `Symbol.func_ptr != nullptr` 断言退出。Prevention：新增或重排任何 `EAvidScriptHostBindingId` 时，接口冻结清单必须机械覆盖静态目录、Runtime dispatcher、WAMR native wrapper/switch、Wasmtime adapter 和两后端 smoke；完整队列前先运行一个同时初始化 WAMR 与 Wasmtime 的 Host ABI 探针，禁止把共享目录编译通过当成后端接线完成。
- 2026-08-29 P57.12C9 Semantic schema 已升级到 13/1.13，但 PreparedSemantic publisher、semantic cache report、PowerShell 集成合同和 architecture checker 仍硬编码 12/1.12，完整 Automation 因同一 `ASBI4403` 出现 18 项级联失败。Prevention：Semantic schema 升级必须从 `SemanticContract` 唯一版本 owner 反向机械检索所有构建、缓存、发布、验证、fixture 和门禁消费者；保留旧版兼容分支，但所有“current”断言在完整 UE Automation 前先由 Frontend .NET host 与全套 PowerShell contract 验证。
...
- 2026-08-29 P57.12C5 fixture 修正 readback 时，把带 `TEXT("schema_version")` 的 C++ 片段塞进一个 PowerShell 双引号 `rg` 正则，转义层破坏了分组并报 `unclosed group`；前后的逐字 golden diff 与 `diff --check` 已独立完成，但该正则证据无效。Prevention：只核对固定源码片段时使用独立 `rg -F -- '<literal>' <confirmed-file>`，不为括号、引号密集的 C++ 文本临时编写正则；需要多个字面值时分别调用，避免 shell 与 regex 双层转义。
- 2026-08-29 P57.12C5 首轮可审计完整 Automation 为 358/361：schema v12 已成为合法版本，但 V8 负向测试仍把 12 当作上界外值；共享 renderer 新增 `AvidLatentAttribute` 后，FName 与 TypedProjectApi 的 deterministic golden fixture 也未同步。Prevention：descriptor schema 升级必须机械检索所有 `above the current maximum` 探针并使用 `Current+1`；共享 facade scaffold 增删后，集中 Gate 前枚举全部 byte-for-byte fixture，审查 generator 产生的 actual diff 后成组刷新，不能只验证新增 latent 专项用例。
- 2026-08-29 P57.12C5 阶段末全量 Automation 再次违反本节已有 P57.12C2 记录，误用 `-log=<absolute-path>` 并额外在 `ExecCmds` 追加 `Quit`；测试虽执行了真实 C# profile 子进程，但没有生成指定证据日志，最终 exit 255，整轮不能计入验收。Prevention：Automation 命令不再现场拼接，必须逐字复用已通过模板 `-ExecCmds="Automation RunTests <filter>" -TestExit="Automation Test Queue Empty" -abslog=<new-unique-path>`；启动前机械检索本节 `-abslog` 记录，启动后 30 秒内断言日志存在，否则立即将该轮标为无效且不得引用结果。
- 2026-08-29 P57.12C5B 首版 latent tamper 用例错误假设共享 descriptor parser 会重算 package hash，并假设 `FLatentActionManager::RemoveActionsForObject` 会同步降低 action 计数；实际 parser 是结构层，canonical/活动反射校验分别属于 emitter regeneration 与 Runtime activation，而 latent action 移除在下一次 manager pass 生效。Prevention：安全测试先逐层标明 parser、canonical emitter、Runtime reflection 与 engine manager 的 owner；涉及 UE 生命周期时先读引擎源码确认同步/延迟语义，再冻结断言。
- 2026-08-29 P57.12C5B 首轮集中 UE 构建发现新增 C++ `TestTrue(... ReferenceSource.Contains(TEXT(...)))` 断言多写一个右括号，Bindings/Runtime 已链接但 Editor 以 C2059 停止。Prevention：新增多层宏/函数断言落盘后逐行核对 `TEXT -> Contains -> TestTrue` 的闭合层级，并在集中构建前对 owned C++ 测试 diff 做一次完整 readback；修复后只进行同一轮 no-clean 增量续跑。
- 2026-08-29 P57.12C5B 首版 Guest facade 识别把 C# `is ... or ...` pattern、`&&` 与 fallback `||` 压入同一个 return 表达式，补丁中间还残留了错误闭括号；Semantic 78/78 已通过，但 Guest 编译以 CS1002/CS1513/CS1525 停止。Prevention：涉及 pattern 与多级布尔优先级的条件必须拆成具名 `bool`，完整方法 readback 后先运行 pinned SDK 的 `dotnet format` 与目标项目编译；禁止把未编译的复合条件直接带入 UE 集中构建。
...
### 2026-08-29: construct every repository .NET command with the pinned host
...
### 2026-08-30: Phase 58 source discovery must use literal paths and simple queries

- Mistake: early P58.A discovery inferred one `BindingInvocation.cpp` subdirectory, passed `*` in two Windows path arguments, named two nonexistent module roots, and submitted grouped regular expressions with unbalanced escaping. The intended sibling results did not make those failed commands valid evidence.
- Prevention: obtain unfamiliar files through one bounded `rg --files` query before reading them. Every later path must be copied literally from that result; filename filtering belongs in a second simple query, and source discovery uses plain alternation without grouped shell escaping unless grouping is essential.

- Mistake: P58.A later guessed a companion `.cpp` for an indexed header even though no such file had been returned, and a broad patch anchor inserted composite properties into an earlier test class with the same `ReadableFString` field. Prevention: a discovered header does not imply a sibling implementation file; query the file index before every companion read. Repeated field names are not valid patch ownership anchors, so UCLASS edits must include the exact class declaration and the next unique member in the patch context, followed by a class-local readback before building.
- Mistake: the first composite heap Automation fixture instantiated abstract `UObject`, reproducing an already documented UE5.8 ensure. Prevention: every `NewObject<T>` in Automation must use a concrete plugin test UCLASS already proven by the owning module; before adding the call, mechanically check both the local test-type index and the existing `abstract UObject` mistake record.
- Mistake: P58.B discovery repeated the path-guessing regression by naming four plausible codec/descriptor files that were not present in the current `rg --files` index. Prevention: a remembered subsystem or class name is never path evidence; every unfamiliar source path must be copied from a successful bounded file-index result in the same work segment before any content query references it.
- Mistake: the first P58.B aggregate Guest test forwarded only `exec_command.output`; when the 30-second yield elapsed, the session handle was discarded and the unfinished run could not be polled. Prevention: every command that may yield must return the complete structured `exec_command` result, capture `session_id`, and poll that exact handle to a final exit before interpreting any partial output.
