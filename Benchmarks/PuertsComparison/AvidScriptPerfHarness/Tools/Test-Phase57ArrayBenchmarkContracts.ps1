...
foreach ($profilePath in $profilePaths) {
    ...
    Assert-True (@($profile.lanes).Count -eq 3) "Lane count mismatch: $profilePath"
...
Assert-True ($runnerCpp.Contains('EAvidScriptVmBackendKind::Wasmtime') -and
    ...
    $runnerCpp.Contains('phase57_array_process_result.v1')) `
...
try {
    ...
    $processResult = [ordered]@{
        contract = 'phase57_array_process_result.v1'
    ...
    Assert-True (@($aggregate.statistics).Count -eq 18 -and @($aggregate.headline).Count -eq 6) `
