	...
	const FString& Parameters)
{
	...
	TestFalse(TEXT("Producer binding has no env compatibility alias"),
		...
		|| IsAvidScriptVmStaticHostImport(TEXT("env"),
			UTF8_TO_TCHAR(AvidScript::TaskResult::Abi::BindProducerImport)));
	TestEqual(TEXT("Task failure propagation appends without renumbering"),
		static_cast<uint16>(TaskPropagateFailure.BindingId),
		static_cast<uint16>(EAvidScriptHostBindingId::TaskBindProducerV1) + 1);
	TestEqual(TEXT("Task failure propagation uses two full-width tokens"),
		FString(UTF8_TO_TCHAR(TaskPropagateFailure.Signature)),
		FString(TEXT("(II)i")));
	TestTrue(TEXT("Task failure propagation requires an invocation scope"),
		TaskPropagateFailure.bRequiresManagedInvocation
		&& RequiresAvidScriptVmManagedInvocation(TEXT("avidscript"),
			UTF8_TO_TCHAR(AvidScript::TaskResult::Abi::PropagateFailureImport)));
	TestFalse(TEXT("Task failure propagation has no env compatibility alias"),
		TaskPropagateFailure.bSupportsEnvCompatibility
		|| IsAvidScriptVmStaticHostImport(TEXT("env"),
			UTF8_TO_TCHAR(AvidScript::TaskResult::Abi::PropagateFailureImport)));
	TestEqual(TEXT("Task continuation retention appends without renumbering"),
		static_cast<uint16>(TaskRetainForContinuation.BindingId),
		static_cast<uint16>(EAvidScriptHostBindingId::TaskPropagateFailureV1) + 1);
	TestEqual(TEXT("Task continuation retention uses two full-width tokens"),
		FString(UTF8_TO_TCHAR(TaskRetainForContinuation.Signature)), FString(TEXT("(II)i")));
	TestTrue(TEXT("Task continuation retention requires an invocation scope"),
		TaskRetainForContinuation.bRequiresManagedInvocation
		&& RequiresAvidScriptVmManagedInvocation(TEXT("avidscript"),
			UTF8_TO_TCHAR(AvidScript::TaskResult::Abi::RetainForContinuationImport)));
	TestFalse(TEXT("Task continuation retention has no env compatibility alias"),
		TaskRetainForContinuation.bSupportsEnvCompatibility
		|| IsAvidScriptVmStaticHostImport(TEXT("env"),
			UTF8_TO_TCHAR(AvidScript::TaskResult::Abi::RetainForContinuationImport)));
	TestEqual(TEXT("Task language-error fault appends without renumbering"),
		static_cast<uint16>(TaskFaultLanguageError.BindingId),
		static_cast<uint16>(EAvidScriptHostBindingId::TaskRetainForContinuationV1) + 1);
	TestEqual(TEXT("Task language-error fault keeps full-width task and object tokens"),
		FString(UTF8_TO_TCHAR(TaskFaultLanguageError.Signature)), FString(TEXT("(IiiI)i")));
	TestEqual(TEXT("Task language-error fault import name"),
		FString(UTF8_TO_TCHAR(TaskFaultLanguageError.ImportName)),
		FString(UTF8_TO_TCHAR(AvidScript::TaskResult::Abi::FaultLanguageErrorImport)));
	TestTrue(TEXT("Task language-error fault requires an invocation scope"),
		TaskFaultLanguageError.bRequiresManagedInvocation
		&& RequiresAvidScriptVmManagedInvocation(TEXT("avidscript"),
			UTF8_TO_TCHAR(AvidScript::TaskResult::Abi::FaultLanguageErrorImport)));
	TestFalse(TEXT("Task language-error fault has no env compatibility alias"),
