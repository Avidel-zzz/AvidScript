...
typedef struct AvidScriptWasmtimeSelfI32Bridge
{
...
typedef struct AvidScriptWasmtimeSelfI32GuestResultBridge
{
	AvidScriptWasmtimeSelfI32GuestResultCallback callback;
	void* environment;
} AvidScriptWasmtimeSelfI32GuestResultBridge;
...
static wasm_trap_t* avidscript_wasmtime_self_i32_guest_result_trampoline(
	void* environment,
	wasmtime_caller_t* caller,
	wasmtime_val_raw_t* args_and_results,
	size_t count)
{
	AvidScriptWasmtimeSelfI32GuestResultBridge* bridge =
		(AvidScriptWasmtimeSelfI32GuestResultBridge*)environment;
	int32_t status = 0;
	(void)caller;
	if (bridge == NULL || bridge->callback == NULL || args_and_results == NULL)
		return avidscript_wasmtime_typed_bridge_unavailable();
...
static wasm_trap_t* avidscript_wasmtime_self_i32_trampoline(
	void* environment,
	wasmtime_caller_t* caller,
	wasmtime_val_raw_t* args_and_results,
	size_t count)
...
{
	AvidScriptWasmtimeSelfI32GuestResultBridge* bridge =
		(AvidScriptWasmtimeSelfI32GuestResultBridge*)calloc(1, sizeof(*bridge));
	if (bridge == NULL)
		return avidscript_wasmtime_local_failure("Could not allocate the typed self-i32 guest-result bridge.");
	bridge->callback = callback;
	bridge->environment = environment;
	return avidscript_wasmtime_linker_define_typed_i32(
		linker,
		...
		import_name_size,
		4,
		avidscript_wasmtime_self_i32_guest_result_trampoline,
		bridge,
