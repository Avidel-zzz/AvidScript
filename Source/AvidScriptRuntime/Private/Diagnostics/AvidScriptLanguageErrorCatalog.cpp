	...
	FString& OutError)
{
	...
	if (!bFound)
	{
		...
		{
			OutError = TEXT("Guest IR 17 WASM is missing language-error metadata");
			return false;
		}
		return true;
	}
	...
	{
		OutError = TEXT("language-error metadata has no matching IR 17 provenance");
		return false;
	}
	...
	if (!CatalogPrivate::Number(*Document, TEXT("schema_version"), 1, 1, SectionVersion)
		|| !CatalogPrivate::Number(*Document, TEXT("guest_ir_schema_version"), 17, 17, GuestSchema)
		|| !Document->TryGetStringField(TEXT("guest_ir_version"), GuestVersion)
		|| GuestVersion != TEXT("1.16")
