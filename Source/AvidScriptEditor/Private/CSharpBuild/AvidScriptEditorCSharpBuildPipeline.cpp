...
bool EnsureAvidScriptGeneratedBindingModule(
	...
	FAvidScriptEditorCSharpBuildResult& OutResult)
{
	...
	{
		SetAvidScriptCSharpBuildPipelineFailure(
			GeneratedResult.ErrorCategory.IsEmpty()
				? FString(TEXT("generated_binding_module_emit_failed"))
				: GeneratedResult.ErrorCategory,
			GeneratedResult.ErrorMessage.IsEmpty()
				? FString::Printf(
					TEXT("Generated binding module could not be emitted for project: %s"),
					*ProjectFile)
				: GeneratedResult.ErrorMessage,
			TEXT("repair the generated binding source error and retry the C# build"),
			OutResult);
		return false;
	}
	...
	FAvidScriptEditorCSharpBuildResult& OutResult)
{
	...
	if (OutPlan.bAutomaticBindingSlice)
	{
		...
		OutResult.bReusedAuthorizationBindingPackage =
			OutPlan.bReusedAuthorizationBindingPackage;
		if (!EnsureAvidScriptGeneratedBindingModule(
				BindingEmitResult,
				OutResult))
		{
			return false;
