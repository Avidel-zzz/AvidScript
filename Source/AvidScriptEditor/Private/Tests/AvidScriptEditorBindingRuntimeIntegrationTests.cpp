	...
	TArray<uint8>& OutBytecode)
{
	...
	Test.TestTrue(
		*FString::Printf(TEXT("%s Tick reads and writes through generated bindings"), *BuildLabel),
		Actor->GetActorScale3D().Equals(FVector(3.5, 3.0, 54.0), 0.001));
	Test.TestEqual(
	...
	Test.TestTrue(
		*FString::Printf(TEXT("%s committed C# Tick retains live reflected writes"), *BuildLabel),
		ScaleBeforeRejectedCandidate.Equals(FVector(3.25, 3.0, 54.0), 0.001));
...
bool FAvidScriptGeneratedCSharpDiagnosticsTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(
		...
		ScaleBeforeRejectedCandidate.Equals(FVector(2.25, 3.0, 54.0), 0.001));
	...
	TestTrue(
		...
		Actor->GetActorScale3D().Equals(FVector(2.5, 3.0, 54.0), 0.001));
	return true;
}
