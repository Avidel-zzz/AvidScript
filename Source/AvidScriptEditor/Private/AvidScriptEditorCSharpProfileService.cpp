	...
	if (Object->HasField(TEXT("self_class_path")))
	{
			...
			&& SchemaVersion != 8)
		{
			SetAvidScriptCSharpProfileFailure(
				...
				TEXT("binding_profile.self_class_path requires C# profile schema_version 3 through 8."),
				TEXT("upgrade the C# profile to schema_version 8 before declaring self_class_path"),
				OutResult);
			return false;
		}
		...
		for (const TSharedPtr<FJsonValue>& ClassValue : *Classes)
		{
				...
				&& SchemaVersion != 8)
			{
				SetAvidScriptCSharpProfileFailure(
					...
					TEXT("binding_profile classes include_events/exclude_events requires C# profile schema_version 8."),
					TEXT("upgrade schema_version to 8 or remove delegate event filters"),
					OutResult);
				return false;
			}
			if (ClassObject->HasField(TEXT("native_direct_functions")))
			{
					...
					&& SchemaVersion != 8)
				{
					SetAvidScriptCSharpProfileFailure(
						...
						TEXT("binding_profile classes native_direct_functions requires C# profile schema_version 6 through 8."),
			...
			if (ClassObject->HasField(TEXT("generated_native_functions")))
			{
				if (SchemaVersion != 7 && SchemaVersion != 8)
				{
					SetAvidScriptCSharpProfileFailure(
						...
						TEXT("binding_profile classes generated_native_functions requires C# profile schema_version 7 or 8."),
			...
			if (ClassObject->HasField(TEXT("generated_native_properties")))
			{
				if (SchemaVersion != 7 && SchemaVersion != 8)
				{
					SetAvidScriptCSharpProfileFailure(
						...
						TEXT("binding_profile classes generated_native_properties requires C# profile schema_version 7 or 8."),
				...
				&& SchemaVersion != 8)
			{
				SetAvidScriptCSharpProfileFailure(
					...
					TEXT("binding_profile classes writable_properties requires C# profile schema_version 5 through 8."),
					TEXT("upgrade schema_version to 8 or remove writable_properties"),
					OutResult);
				return false;
			}
	...
	if (Object->HasField(TEXT("object_factories")))
	{
			...
			&& SchemaVersion != 8)
		{
			SetAvidScriptCSharpProfileFailure(
				...
				TEXT("C# binding_profile object_factories requires profile schema_version 4 through 8."),
				TEXT("upgrade the C# profile to schema_version 8 before declaring object_factories"),
				OutResult);
			return false;
		}
	...
	FAvidScriptEditorCSharpProfileLoadResult& OutResult)
{
			...
			&& SchemaVersion != 8.0))
	{
		SetAvidScriptCSharpProfileFailure(
			...
			TEXT("C# profile schema_version must be an integer from 1 through 8."),
			TEXT("update the profile JSON to schema_version 8"),
			OutResult);
		return false;
	}
		...
		&& bHasObjectFactories)
	{
		SetAvidScriptCSharpProfileFailure(
			TEXT("binding_profile_factory_schema_unsupported"),
			TEXT("binding_profile.object_factories requires C# profile schema_version 4 through 8."),
			TEXT("upgrade schema_version to 8 or remove object_factories"),
			OutResult);
		return false;
	}
