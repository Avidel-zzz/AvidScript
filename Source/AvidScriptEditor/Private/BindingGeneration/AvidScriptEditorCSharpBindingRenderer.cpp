	...
	FString& OutErrorSource)
{
			...
			OutErrorSource))
	{
		...
		{
			OutErrorSource = Type.ElementTypeId;
		}
		return false;
	}
...
void AppendAsyncAwaitableReferenceSurface(TArray<FString>& Lines)
{
	Lines.Append({
		...
		TEXT("{"),
		TEXT("    public AvidDelayAwaitable WithCancellation(AvidCancellationToken token) => default;"),
		TEXT(""),
		TEXT("    public AvidDelayAwaiter GetAwaiter() => default;"),
		TEXT("}"),
		TEXT(""),
		TEXT("public readonly struct AvidDelayAwaiter : INotifyCompletion"),
		TEXT("{"),
		...
		TEXT("    }"),
		TEXT(""),
		TEXT("    public void GetResult()"),
		TEXT("    {"),
		TEXT("    }"),
		TEXT("}"),
		TEXT(""),
		TEXT("public readonly struct AvidObjectAwaitable"),
		TEXT("{"),
		TEXT("    public AvidObjectAwaitable WithCancellation(AvidCancellationToken token) => default;"),
		TEXT(""),
		TEXT("    public AvidObjectAwaiter GetAwaiter() => default;"),
		TEXT("}"),
		TEXT(""),
		TEXT("public readonly struct AvidObjectAwaiter : INotifyCompletion"),
		TEXT("{"),
		TEXT("    public bool IsCompleted => false;"),
		TEXT(""),
		TEXT("    public void OnCompleted(Action continuation)"),
...
void AppendContinuationReferenceSurface(TArray<FString>& Lines)
{
	Lines.Append({
		...
		TEXT(""),
		TEXT("[StructLayout(LayoutKind.Sequential)]"),
		TEXT("public readonly struct AvidCancellationToken"),
		TEXT("{"),
		TEXT("    internal readonly long Value;"),
		TEXT(""),
		TEXT("    internal AvidCancellationToken(long value)"),
		TEXT("    {"),
		TEXT("        Value = value;"),
		TEXT("    }"),
		TEXT(""),
		TEXT("    public bool IsValid => Value != 0;"),
		TEXT("}"),
		TEXT(""),
		TEXT("[StructLayout(LayoutKind.Sequential)]"),
		TEXT("public readonly struct AvidCancellationSource"),
		TEXT("{"),
		TEXT("    private readonly long Value;"),
		TEXT("    public readonly AvidCancellationToken Token;"),
		TEXT(""),
		TEXT("    private AvidCancellationSource(long value)"),
		TEXT("    {"),
		TEXT("        Value = value;"),
		TEXT("        Token = new AvidCancellationToken(value);"),
		TEXT("    }"),
		TEXT(""),
		TEXT("    public static AvidCancellationSource Create()"),
		TEXT("        => new(AvidScriptRuntimeNative.ContinuationCancelSourceCreate());"),
		TEXT(""),
		TEXT("    public bool IsValid => Value != 0;"),
		TEXT("    public bool Cancel()"),
		TEXT("        => AvidScriptRuntimeNative.ContinuationCancelSourceCancel(Value) != 0;"),
		TEXT("    public bool Release()"),
		TEXT("        => AvidScriptRuntimeNative.ContinuationCancelSourceRelease(Value) != 0;"),
		TEXT("}"),
		TEXT(""),
		...
		TEXT(""),
		TEXT("    public static AvidDelayAwaitable DelayAsync(float delaySeconds)"),
		TEXT("        => default;"),
		TEXT(""),
		...
		TEXT(""),
		TEXT("    public static AvidDelayAwaitable NextTickAsync()"),
		TEXT("        => default;"),
		TEXT("}"),
		TEXT(""),
		TEXT("public static class AvidAssets"),
		TEXT("{"),
		TEXT("    public static AvidObjectAwaitable LoadObjectAsync(string assetPath)"),
		TEXT("        => default;"),
		TEXT(""),
	...
	FString& OutErrorSource)
{
	...
	AppendAsyncAwaitableReferenceSurface(Lines);
	Lines.Append({
		...
		TEXT(""),
		TEXT("    [DllImport(\"avidscript\", EntryPoint = \"avid_continuation_delay_cancel_resume_v1\")]"),
		TEXT("    internal static extern long ContinuationDelayCancelResumeV1(float delaySeconds, int callbackId);"),
		TEXT(""),
		...
		TEXT(""),
		TEXT("    [DllImport(\"env\", EntryPoint = \"continuation_cancel_source_create\")]"),
		TEXT("    internal static extern long ContinuationCancelSourceCreate();"),
		TEXT(""),
		TEXT("    [DllImport(\"env\", EntryPoint = \"continuation_cancel_source_cancel\")]"),
		TEXT("    internal static extern int ContinuationCancelSourceCancel(long sourceToken);"),
		TEXT(""),
		TEXT("    [DllImport(\"env\", EntryPoint = \"continuation_cancel_source_release\")]"),
		...
		for (int32 ArrayIndex = 0; ArrayIndex < ArrayTypes.Num(); ++ArrayIndex)
		{
			...
			if (!ResolveArrayCSharpType(
					*ArrayTypes[ArrayIndex],
					...
					ArrayType,
					OutErrorSource)
				|| ReleaseSignatures.Contains(ArrayType))
			{
				OutErrorCategory = TEXT("csharp_type_collision");
			...
			Lines.Add(FString::Printf(
				TEXT("    public static bool Release(%s value) => AvidScriptNative.ReleaseArray%04d(value) != 0;"),
				*ArrayType,
				ArrayIndex));
		}
		Lines.Append({ TEXT("}"), TEXT("") });
